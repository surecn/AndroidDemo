...
public class SrsFlvMuxer {
    ...
    private LinkedList<SrsFlvFrame> frameCache = new LinkedList<SrsFlvFrame>();
    private boolean isDropping = false;
    private int nInput = 0;
    private int nOutput = 0;
    private int nLastInput = 0;
    private int nLastOutput = 0;
    private int upCount = 0;
    private final int kBitrateDownupCycle = 65 * 3;
    private final int kMaxLengthDrop = 65 * 5;
    private final int kMaxLengthClean = 65 * 10;
    private final int kCheckFrameNum = 65 * 5;
...
//        return publisher == null ? null : publisher.getVideoFrameCacheNumber(); // jiarong
    ...
    public void setVideoResolution(int width, int height) {
        if (publisher != null) {
//            publisher.setVideoResolution(width, height); // jiarong
        }
    }
...
//        }
        publisher.rtmpSend(frame.tag.frame.array());
                ...
                while (!Thread.interrupted()) {
                    ...
                    while (true) {
                        ...
                        synchronized (frameCache) {
                            ...
                            frame = frameCache.poll();
                            nOutput++;
                        }
                        try {
                            // only connect when got keyframe.
//                            if (frame.is_keyframe()) {
//                                connect(streamId,url,appId,appSecretKey,appUid,appUidTokenMd5);
//                            }
...
//            }
    ...
    class SrsFlv {
        ...
        private void rtmp_write_packet(int type, int dts, int frame_type, int avc_aac_type, SrsFlvFrameBytes tag) {
            ...
            synchronized (frameCache) {
                nInput++;
//                Log.i(TAG, String.format("++++++++ Video bitrate Change: nInput = %d", nInput)); // jiarong

                if (isDropping && frame.is_video()) {
                    if (frame.is_keyframe()) {
                        isDropping = false;
                    ...
                    int size = frameCache.size();
//                    Log.i(TAG, String.format("++++++++ Video bitrate Change: size = %d", size)); // jiarong
                    if ((nInput % kCheckFrameNum == 0) && size > kMaxLengthDrop) {
                        //handler.onRtmpDropFrames();
                        if (size < kMaxLengthClean) {
                            Log.i(TAG, "voiptest, dropFrames");
                            dropFrames();
                        } else {
                            Log.i(TAG, "voiptest, cleanFrames");
                            cleanFrames();
                        }
                    }

                    // dynamic video bitrate
                    if (nInput % kBitrateDownupCycle == 0) {
                        int nInputLastPeriod = nInput - nLastInput;
                        int nOutputLastPeriod = nOutput - nLastOutput;
                        if (nInputLastPeriod > nOutputLastPeriod) {
                            upCount = 0;
                            float delayRate = (nInputLastPeriod - nOutputLastPeriod) * 1.0f / nInputLastPeriod;
                            if (delayRate > 0.1 || size > 30) {
                                // down
                                //handler.onRtmpVideoBitrateChange(true);
                            }
                        } else {
                            if (size < 2) {
                                upCount++;
                                if (upCount >= 3) {
                                    upCount = 0;
                                    // up
                                    //handler.onRtmpVideoBitrateChange(false);
                                }
                            } else {
                                upCount = 0;
                            }
...
//            AZusLog.i(TAG, String.format("jiarong flv: enqueue frame size=%d", frameCache.size()));
        }

        private void dropFrames() {
            LinkedList<SrsFlvFrame> tmpList = new LinkedList<SrsFlvFrame>();

            boolean hasKeyFrame = true;
            int backKeyFrameIndex = -1;
            int index = 0;
            SrsFlvFrame item = frameCache.poll();
            while (item != null) {
                if (item.is_keyframe()) {
                    backKeyFrameIndex = index;
                }

                tmpList.add(item);
                index++;
                item = frameCache.poll();
            }
            if (backKeyFrameIndex == -1) {
                backKeyFrameIndex = tmpList.size();
                hasKeyFrame = false;
            }
            ...
            index = 0;
            while (item != null) {
                if (item.is_video() && index < backKeyFrameIndex) {
                    if (item.avc_aac_type == SrsCodecVideoAVCType.SequenceHeader) {
                        frameCache.add(item);
                    }
                } else {
                    frameCache.add(item);
