package com.surecn.demo.opengles.data;
...
import static com.surecn.demo.opengles.Constants.BYTES_PER_SHORT;
...
public class IndexBuffer {
    ...
    public IndexBuffer(short[] vertexData) {
        ...
        glGenBuffers(buffers.length, buffers, 0);//创建新的缓冲区
        if (buffers[0] == 0) {
            throw new RuntimeException("Could not create a new vertex buffer object.");
        }
        bufferId = buffers[0];

        glBindBuffer(GL_ELEMENT_ARRAY_BUFFER, buffers[0]);//绑定这个缓冲区的id
